...
import (
	...
	hermes "github.com/beyond5959/ngent/internal/agents/hermes"
)

// TestPreflight verifies that Preflight returns nil when the hermes-acp binary exists.
func TestPreflight(t *testing.T) {
	if _, err := exec.LookPath("hermes-acp"); err != nil {
		if _, err := exec.LookPath("hermes"); err != nil {
			t.Skip("hermes-acp and hermes not in PATH")
		}
	}
	if err := hermes.Preflight(); err != nil {
		t.Fatalf("Preflight() = %v, want nil", err)
	}
}

// TestStreamWithFakeProcess tests the Stream protocol using a fake hermes binary.
...
for line in sys.stdin:
    ...
    if method == "initialize":
        send({"jsonrpc":"2.0","id":rid,"result":{
            "protocolVersion":1,
            "agentInfo":{"name":"hermes","title":"Hermes","version":"0.1.0"},
            "authMethods":[],
...
`, python3)
	...
	c, err := hermes.New(hermes.Config{Dir: tmpDir})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
...
for line in sys.stdin:
    ...
    if method == "initialize":
        ...
        send({"jsonrpc":"2.0","id":rid,"result":{
            "sessionId":"ses_hermes_model_123",
            "models":{"currentModelId":"test-model","availableModels":[]}
        }})
    elif method == "session/prompt":
...
`, python3)
	...
	c, err := hermes.New(hermes.Config{
		Dir:     tmpDir,
		ModelID: "hermes-pro",
	})
	if err != nil {
...
for line in sys.stdin:
    ...
    if method == "initialize":
        ...
        send({"jsonrpc":"2.0","method":"session/update","params":{
            "sessionId":"ses_hermes_slash_123",
            "update":{
...
for line in sys.stdin:
    ...
    if method == "initialize":
        ...
        send({"jsonrpc":"2.0","method":"session/update","params":{
            "sessionId":"ses_hermes_config_slash_123",
            "update":{
                "sessionUpdate":"available_commands_update",
                "availableCommands":[
            ...
            "configOptions":[
                {
                    ...
                    "name":"Model",
                    "type":"select",
                    "currentValue":"hermes-pro",
                    "options":[
                        {"value":"hermes-pro","name":"Hermes Pro"}
                    ]
                }
            ]
        }})
...
for line in sys.stdin:
    ...
    if method == "initialize":
        ...
        send({"jsonrpc":"2.0","id":rid,"result":{
            "sessionId":"ses_hermes_models_123",
            "models":{
                "currentModelId":"hermes-pro",
                "availableModels":[
                    "hermes-pro",
                    {"modelId":"hermes-max","name":"Hermes Max"}
                ]
            }
        }})
`, python3)
	...
	models, err := hermes.DiscoverModels(context.Background(), hermes.Config{Dir: tmpDir})
	if err != nil {
	...
	if models[0].ID != "hermes-pro" {
		t.Fatalf("models[0].id = %q, want %q", models[0].ID, "hermes-pro")
	}
	if models[1].ID != "hermes-max" {
		t.Fatalf("models[1].id = %q, want %q", models[1].ID, "hermes-max")
	}
}
...
// TestHermesE2ESmoke performs a real turn with the installed hermes binary.
// Run with: E2E_HERMES=1 go test ./internal/agents/hermes/ -run E2E -v -timeout 90s
func TestHermesE2ESmoke(t *testing.T) {
	if os.Getenv("E2E_HERMES") != "1" {
		t.Skip("set E2E_HERMES=1 to run")
	}
	if err := hermes.Preflight(); err != nil {
		t.Skipf("hermes not available: %v", err)
	}
	...
	c, err := hermes.New(hermes.Config{Dir: cwd})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
...
func TestHermesE2ESessionTranscriptReplay(t *testing.T) {
	if os.Getenv("E2E_HERMES") != "1" {
		t.Skip("set E2E_HERMES=1 to run")
	}
	if err := hermes.Preflight(); err != nil {
	...
	marker := fmt.Sprintf("HERMES_TRANSCRIPT_%d", time.Now().UnixMicro())
