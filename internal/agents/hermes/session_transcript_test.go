...
for line in sys.stdin:
    ...
    if method == "initialize":
        ...
        send({"jsonrpc":"2.0","id":rid,"result":{
            "sessions":[{"sessionId":"ses_hermes_history","cwd":params.get("cwd",""),"title":"History"}]
        }})
    elif method == "session/load":
        sid = params.get("sessionId","")
        send({"jsonrpc":"2.0","method":"session/update","params":{
            "sessionId":sid,
            "update":{"sessionUpdate":"user_message_chunk","content":{"type":"text","text":"hello hermes"}}
        }})
        send({"jsonrpc":"2.0","method":"session/update","params":{
            "sessionId":sid,
            "update":{"sessionUpdate":"tool_call_update","content":[{"type":"content","content":{"type":"text","text":"tool output"}}]}
        }})
        send({"jsonrpc":"2.0","method":"session/update","params":{
            "sessionId":sid,
...
`, python3)
	...
	client, err := hermes.New(hermes.Config{Dir: tmpDir})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	...
	if got := result.Messages[0]; got.Role != "user" || got.Content != "hello hermes" {
		t.Fatalf("messages[0] = %+v, want user hello hermes", got)
	}
